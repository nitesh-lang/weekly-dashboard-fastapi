...
import { Brain, ChevronDown, ChevronUp, TrendingUp, TrendingDown, AlertTriangle, CheckCircle2, Target, Activity, Boxes, BarChart3 } from "lucide-react";

/* ─────────────────────────────────────────────────────────────────
   Senior-PPC insights panel — consumes /api/ams/insights and renders
   it as a categorised, color-coded brief.

   Visual approach: NOT another bland card grid.  Each section has a
   left rail in its category's accent color, the heading is set in
   small-caps mono with letter-spacing to match the Operator-Console
   aesthetic, and each insight rendered as a single-line statement
   with markdown-style **bold** rendered inline.

   The insight kind drives the dot/rail color:
       positive  → emerald
       negative  → rose
       warning   → amber
       neutral   → slate
   ...
   Empty sections collapse silently — no padding, no rendered noise.
   Loading state is intentionally subtle: a 1-line "Reading W##-W##…"
   so the panel never flashes a full skeleton between filter clicks.
   ───────────────────────────────────────────────────────────────── */
...
const SECTION_META: Record<string, { label: string; icon: React.ComponentType<{ className?: string }> }> = {
    ...
    inventory:  { label: "Inventory ✕ Ads", icon: Boxes },
    trajectory: { label: "Trajectory", icon: Activity },
    actions:    { label: "Next actions", icon: CheckCircle2 },
};

/** Render markdown-style **bold** segments without pulling in a full
 *  markdown parser.  Operates on the trusted backend text only — no
 *  user-supplied input flows through here. */
...
export function AmsInsightsPanel({ qs }: { qs: string }) {
    const [open, setOpen] = useState(true);

    const { data, isLoading, error } = useQuery<InsightsPayload>({
        queryKey: ["ams-insights", qs],
        queryFn: () => api.get(`/api/ams/insights${qs ? `?${qs}` : ""}`),
        staleTime: 30_000,
    });
    ...
    return (
        <Card className="overflow-hidden">
            {/* Header strip — distinctive: rotated micro-label + indigo accent.
                The 'Senior analyst read' framing makes the source explicit. */}
            <div
                className="flex items-center gap-3 px-4 py-3 border-b border-slate-200/80"
                style={{ background: "linear-gradient(135deg, #f5f3ff 0%, #fafaff 60%, #ffffff 100%)" }}
            >
                ...
                <div className="flex flex-col">
                    ...
                    <div
                        className="text-[10px] font-medium uppercase text-indigo-700 tabular-nums"
                ...
                <div className="p-4">
                    {error && (
                        <div className="text-[12px] text-rose-600 px-3 py-2 rounded bg-rose-50 border border-rose-200">
                            Couldn't load the read — endpoint returned an error.  Refresh or check the network tab.
                        </div>
                    )}
                    ...
                    {!error && data && totalCount === 0 && (
                        <div className="text-[12px] text-slate-500 px-3 py-4">
                            {data.empty_reason || "No insights for the current selection — widen the filters."}
                        </div>
                    )}
                    {!error && data && totalCount > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div className="space-y-4">
                                ...
                                <Section name="efficiency" items={data.efficiency} />
                                <Section name="trajectory" items={data.trajectory} />
                            </div>
                            {/* Right column — what to act on */}
                            <div className="space-y-4">
