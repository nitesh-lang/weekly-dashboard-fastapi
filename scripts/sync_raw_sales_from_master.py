...
def _sync_workbook(path: Path, asin_cols: tuple[str, ...]) -> dict:
    ...
    blank = {"cells": 0, "orphans": 0, "missing_asin": 0, "orphan_log": [], "missing_log": []}
    if not path.exists():
        return blank
    try:
    ...
    for row in range(2, ws.max_row + 1):
        ...
        if not asin:
            # ASIN-less row — record so the operator can investigate.
            # Skip totally empty rows (no SKU, no Model either) to avoid
            # flagging blank trailing rows from the export.
    ...
    if cell_updates:
        ...
        try:
            ...
            return {"cells": 0, "orphans": len(orphan_log), "missing_asin": len(missing_log),
                    "orphan_log": orphan_log, "missing_log": missing_log}

    return {"cells": cell_updates, "orphans": len(orphan_log),
            "missing_asin": len(missing_log),
            "orphan_log": orphan_log, "missing_log": missing_log}
    ...
    for bdir in BRAND_DIRS:
        ...
        if p.exists():
            ...
            for ln in res["orphan_log"]:
                orphan_report.append({"Week": wk, "File": f"{bdir}/amazon_sales.xlsx", "Detail": ln})
            for ln in res["missing_log"]:
                missing_report.append({"Week": wk, "File": f"{bdir}/amazon_sales.xlsx", "Detail": ln})
...
print("=" * 70)
print(f"✅ {total_cells} cell(s) updated across {len(weeks_in_scope)} weeks · 4 brands")
print()
